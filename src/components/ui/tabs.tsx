...
>(({ className, children, value, ...props }, ref) => {
  ...
  React.useLayoutEffect(() => {
    ...
    const activeTab = listRef.current.querySelector<HTMLButtonElement>(`[data-state=active]`);
    if (activeTab) {
      const { offsetLeft, offsetWidth } = activeTab;
       gsap.to(listRef.current?.querySelector('.glider'), {
        duration: 0.4,
        left: offsetLeft,
        width: offsetWidth,
        ease: "power3.inOut",
  ...
  return (
    ...
    >
      {children}
      <div className="glider absolute bottom-0 left-0 h-0.5 bg-primary" />
    </TabsPrimitive.List>
  );
});
